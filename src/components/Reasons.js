...
const Reasons = () => {
  ...
  return (
    <section className="py-5 px-4 sm:px-10 md:px-20">
      ...
      >
        {/* Overlay - Updated to turquoise tones */}
        <div className="absolute inset-0 bg-gradient-to-br from-primary-600/90 via-primary-700/80 to-primary-800/90 backdrop-blur-[2px]"></div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 font-roboto">
          {reasonsList.map((reason, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: index * 0.1 }}
              className="bg-white/20 backdrop-blur-md p-6 rounded-xl shadow-md hover:shadow-lg hover:bg-white/30 transform hover:-translate-y-1 transition-all duration-300 text-white"
            >
              <div className="w-12 h-12 mx-auto mb-4 rounded-full bg-gradient-to-br from-primary-300 to-primary-500 flex items-center justify-center text-white shadow-md">
                {reason.icon}
              </div>
              <h5 className="font-medium text-sm md:text-base">{reason.text}</h5>
            </motion.div>
