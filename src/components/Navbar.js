  ...
  return (
    <>
      <nav className="bg-white px-4 py-4 sticky top-0 z-50 shadow-md font-poppins flex items-center justify-between flex-wrap lg:px-10">
        ...
        <div className="hidden lg:flex items-center space-x-6 mx-auto" ref={dropdownRef}>
          <NavLink to="/" className={({ isActive }) => `transition ${isActive ? "text-primary-600 font-semibold" : "text-gray-800 hover:text-primary-600"}`}>Home</NavLink>
          <NavLink to="/about" className={({ isActive }) => `transition ${isActive ? "text-primary-600 font-semibold" : "text-gray-800 hover:text-primary-600"}`}>AboutUs</NavLink>

          {/* SERVICES DROPDOWN - TEAL THEMED */}
          <div className="relative">
            <div onClick={() => toggleDropdown("services")} className="flex items-center gap-1 cursor-pointer text-gray-800 hover:text-primary-600 transition font-medium">
              Services {activeDropdown === "services" ? <ChevronUp size={20} className="text-primary-600" /> : <ChevronDown size={20} />}
            </div>
            {activeDropdown === "services" && (
              <div className="absolute left-1/2 top-full transform -translate-x-1/2 mt-2 w-[900px] max-h-[75vh] overflow-y-auto bg-white text-gray-800 rounded-xl shadow-2xl grid grid-cols-4 gap-6 p-6 z-50 border border-primary-100">
                {serviceGroups.map((group) => (
                  <div key={group.heading} className="space-y-3">
                    <h4 className="font-bold text-primary-600 mb-4 pb-3 border-b-2 border-primary-200 sticky top-0 bg-white z-10 text-sm uppercase tracking-wide">
                      {group.heading}
                    </h4>
                    <ul className="space-y-2">
                      {group.items.map(({ to, text }) => (
                        <li key={to}>
                          <NavLink 
                            to={to} 
                            className="block px-3 py-2 rounded-lg text-gray-700 hover:bg-primary-50 hover:text-primary-600 hover:font-semibold transition duration-200"                
                            onClick={() => setActiveDropdown(null)}
                          >
                            {text}
                          </NavLink>
          ...
          <NavLink to="/contact" className={({ isActive }) => `transition ${isActive ? "text-primary-600 font-semibold" : "text-gray-800 hover:text-primary-600"}`}>ContactUs</NavLink>
        </div>

        <div className="flex items-center gap-3 ml-auto">
          <Location />
          <NavLink to="/checkout" className="relative">
            <ShoppingCart size={24} className="text-gray-700 hover:text-primary-600 transition" />
            {cartItemCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full min-w-[20px] h-5 flex items-center justify-center px-1">
                {cartItemCount}
              </span>
...
{!localStorage.getItem("token") ? (
  <button onClick={() => setShowUserSignup(true)} className="flex items-center gap-2 px-4 py-2 rounded-full border border-primary-600 text-sm text-primary-600 hover:bg-primary-50 transition duration-300">
    <FaUser /> Login
  </button>
) : (
  <button onClick={async () => {
  ...
  }} className="flex items-center gap-2 px-4 py-2 rounded-full border border-red-600 text-sm text-red-600 hover:bg-red-50 transition duration-300">
    Logout
  </button>
)}
        </div>
      ...
      <div className={`lg:hidden fixed top-0 right-0 h-full w-64 bg-white shadow-md transform transition-transform duration-300 z-50 ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <ul className="px-4 space-y-4">
          <li><NavLink to="/" onClick={() => setIsOpen(false)} className="block px-2 py-2 hover:bg-primary-50 text-primary-600 font-medium">Home</NavLink></li>
          <li><NavLink to="/about" onClick={() => setIsOpen(false)} className="block px-2 py-2 hover:bg-primary-50 text-primary-600 font-medium">AboutUs</NavLink></li>
          <li>
            <div onClick={() => setMobileDropdownOpen(!mobileDropdownOpen)} className="flex items-center justify-between px-2 py-2 cursor-pointer text-primary-600 font-medium">
              Services {mobileDropdownOpen ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
            </div>
            {mobileDropdownOpen && (
              <ul className="pl-4 space-y-2 max-h-64 overflow-y-auto border-l-2 border-primary-200">
                {serviceGroups.flatMap((group) => group.items).map(({ to, text }) => (
                  <li key={to}>
                    <NavLink to={to} onClick={() => { setIsOpen(false); setMobileDropdownOpen(false); }} className="block px-4 py-2 hover:bg-primary-50 hover:text-primary-600 transition text-gray-700">
                      {text}
                    </NavLink>
                  </li>
                ))}
          ...
          <li><NavLink to="/contact" onClick={() => setIsOpen(false)} className="block px-2 py-2 hover:bg-primary-50 text-primary-600 font-medium">ContactUs</NavLink></li>
        </ul>
      </div>
