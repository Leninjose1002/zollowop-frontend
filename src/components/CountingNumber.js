  ...
  return (
    ...
    >
      <section
        ...
        style={{ backgroundImage: "url('https://maidwala.in/img/fun-bg.jpg')" }}
      >
        <div className="absolute inset-0 bg-gradient-to-br from-gray-900/80 via-primary-900/40 to-gray-900/80"></div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 text-center">
            {stats.map((stat, index) => (
              ...
              >
                <i
                  className={`fas ${stat.icon} text-4xl sm:text-5xl text-primary-400`}
                  aria-hidden="true"
                ></i>
                <div>
                  <h3 className="text-3xl font-bold font-poppins text-primary-300">
                    <CountUp end={stat.target} duration={2} />+
                  </h3>
                  <p className="text-base font-roboto text-gray-200">{stat.label}</p>
                </div>
              </motion.div>
            ))}
          </div>
